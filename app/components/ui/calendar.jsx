...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn(
        "tw:p-2 tw:[--cell-radius:var(--radius-md)] tw:[--cell-size:--spacing(7)] tw:bg-background tw:group/calendar tw:in-data-[slot=card-content]:bg-transparent tw:in-data-[slot=popover-content]:bg-transparent",
      ...
      classNames={{
        ...
        dropdown_root: cn(
          "tw:relative tw: tw:rounded-(--cell-radius)",
        ...
        caption_label: cn("tw:select-none tw:font-medium", captionLayout === "label"
          ? "tw:text-sm"
          : "tw: tw:rounded-(--cell-radius) tw:flex tw:items-center tw:gap-1 tw:text-sm tw: tw:[&>svg]:text-muted-foreground tw:[&>svg]:size-3.5", defaultClassNames.caption_label),
        ...
        week_number_header: cn("tw:select-none tw:w-(--cell-size)", defaultClassNames.week_number_header),
        week_number: cn(
        ...
        range_start: cn(
          "tw:rounded-l-(--cell-radius) tw:bg-muted tw:relative tw:after:bg-muted tw:after:absolute tw:after:inset-y-0 tw:after:w-4 tw:after:right-0 tw:z-0 tw:isolate",
      ...
      components={{
        ...
        Chevron: ({ className, orientation, ...props }) => {
          if (orientation === "left") {
            return (<ChevronLeftIcon className={cn("tw: tw:size-4", className)} {...props} />);
          }

          if (orientation === "right") {
            return (<ChevronRightIcon className={cn("tw: tw:size-4", className)} {...props} />);
          }
        ...
        WeekNumber: ({ children, ...props }) => {
          return (
            <td {...props}>
              <div
                className="tw:flex tw:size-(--cell-size) tw:items-center tw:justify-center tw:text-center">
                {children}
              </div>
            </td>
          );

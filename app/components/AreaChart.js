...
export default function AreaChart({ title, series, height = 240, xLabels = null }) {
  const width = 640;
  const padding = { top: 20, right: 20, bottom: 40, left: 60 };
  const normalized = normalizeSeries(series, width, height, padding);

  // Get min/max from first series
      ...
      <div className="chart-frame">
        ...
        <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="auto">
          <defs>
            ...
            {series.map((item, idx) => (
              <linearGradient key={item.name} id={`areaGrad-${idx}`} x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          {/* Background */}
          <rect x="0" y="0" width={width} height={height} fill="url(#areaBg)" rx="8" />
          ...
          {normalized.map((item, idx) => (
            <g key={item.name}>
              <path
                d={makeAreaPath(item.points, height, padding)}
